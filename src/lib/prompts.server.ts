// Centralized system prompts for tailoring / generation tasks.

export const RESUME_TAILOR_SYSTEM = `You are a world-class ATS resume writer
...
export const ANSWER_SYSTEM = `You are helping a job seeker answer a single
...
export const REVIEWER_SYSTEM = `You are a senior hiring manager and resume
...
export const EXPAND_SYSTEM = `You are extracting a candidate's professional
...
export const UPSKILL_SYSTEM = `You analyze a job posting against a candidate's
...
export const INTERVIEW_SYSTEM = `You are an interview coach. Given a job and
...
export const STYLE_ANALYZE_SYSTEM = `You analyze writing samples to extract a
...
export const BEHAVIORAL_SYSTEM = `You are a behavioral-interview coach. Build
...
export const REFINE_SYSTEM = `You are revising an existing application
...
export const SALARY_SYSTEM = `You are a compensation analyst. Estimate a
realistic salary band (base, total comp if equity is common) for the role at
the named company in the named location, given the candidate's seniority and
the job's requirements. Cite the reasoning (market signals you used, company
tier, location adjustment). Then produce a negotiation script the candidate
can use, including: opening anchor, justification, counter-offer framing,
walk-away. Return JSON exactly matching the schema.`;

export const CAREER_SYSTEM = `You are a career strategist. Given a
...
export const STYLE_DIRECTIVE_TEMPLATE = `WRITING STYLE TO MATCH:
{style}
